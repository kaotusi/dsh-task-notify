...
function apply(ctx) {
  ...
  ctx.on('subagent/end', (info) => {
    try {
      ...
      const detail = (typeof info.provider === 'string' ? info.provider : 'subagent') + ' · ' + String(info.stopReason || 'ended')
      push(info.runId ? 'sub:' + String(info.runId) : undefined, 'task-end', 'subagent', '', detail, ok ? 'ok' : 'failed')
  ...
  ctx.tools.register(defineTool({
    name: 'ntfy_status',
    description: 'Read the task-notify plugin client diagnostics: whether the browser supports the Notification API, the current permission state, secure-context status, and the last report time.',
    parameters: {},
    output: {
      schema: {
        type: 'object',
    ...
    execute() {
      return clientDiag === null ? { reported: false } : clientDiag
    },
  }))
}
