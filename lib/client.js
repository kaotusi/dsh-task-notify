  ...
  factory: (require) => {
    ...
    async function apply(ctx) {
      ...
      const ingest = (list) => {
        ...
        if (state.toasts.some((id) => !alive.has(id))) {
          state.toasts = state.toasts.filter((id) => alive.has(id))
          changed = true
        }
        // Budget: at most 3 new OS pushes per poll round, so a burst of
        // unpushed items (fresh storage, new tab) trickles in instead of
        // firing one banner per item.
        let budget = 3
        let pushedNow = false
      ...
      const Poller = () => {
        React.useEffect(() => {
          ...
          // Background tabs skip the 1.5s full-poll churn; returning to the
          // foreground polls immediately instead of waiting for the next tick.
          const onVis = () => { if (!pageHidden()) poll() }
          let removeVis = null
          try {
            document.addEventListener('visibilitychange', onVis)
            removeVis = () => document.removeEventListener('visibilitychange', onVis)
          } catch (e) { /* ignore */ }
          return () => { alive = false; disposer(); if (removeVis !== null) removeVis() }
        }, [])
        return null
      }
